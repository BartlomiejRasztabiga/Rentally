...
class CRUDReservation(CRUDBase[Reservation, ReservationCreateDto, ReservationUpdateDto]):
    ...
    def validate_collisions(self, db: Session, obj: Union[ReservationCreateDto, ReservationUpdateDto, Reservation]):
    ...
    def create(self, db: Session, *, obj_in: ReservationCreateDto) -> Reservation:
        ...
        # TODO cannot reserve a car if there is already another reservation or rental for the same time frame
        self.validate_collisions(db, obj_in)

        obj_in.status = ReservationStatus.NEW
        return super().create(db=db, obj_in=obj_in)

    def get_active_by_car_id(self, db: Session, car_id) -> List[Reservation]:
        return db.query(Reservation).filter(
            Reservation.car_id == car_id and Reservation.status in [ReservationStatus.NEW,
                                                                    ReservationStatus.COLLECTED]).all()
